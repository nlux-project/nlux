...
def _year_timespan(year):
    timespan = model.TimeSpan()
    year = str(year).zfill(4)
    timespan.begin_of_the_begin = f"{year}-01-01T00:00:00"
    timespan.end_of_the_end = f"{year}-12-31T23:59:59"
    return timespan


def _event_with_year(event_cls, year):
    event = event_cls()
    event.timespan = _year_timespan(year)
    return event


def _split_agent_values(value):
    value = _clean(value)
    if not value:
        return []
    ...
    split_parts = []
    life_date_pattern = re.compile(r".+?\*\s*\d{3,4}\s*†\s*\d{3,4}(?=\s+[^,\s]+,\s+|$)")
    for part in parts:
        matches = [match.group(0).strip(" ,;") for match in life_date_pattern.finditer(part)]
        split_parts.extend(matches or [part])
    return split_parts


def _person_from_agent_value(value):
    value = _clean(value)
    if not value:
        return None
    value = value.rstrip(",; ")
    ...
    life_dates = re.search(
        r"^(?P<name>.*?)\s*,?\s*\*\s*(?P<birth>\d{3,4})\s*†\s*(?P<death>\d{3,4})\s*$",
        value,
    )
    if life_dates:
        name = life_dates.group("name").strip(" ,;")
        birth_year = life_dates.group("birth")
        death_year = life_dates.group("death")
    else:
        name = re.sub(r"\s*,?\s*(?:\(?\d{3,4}\b.*|\*.*)$", "", value).strip(" ,;")

    if not name:
        return None

    person = model.Person(label=name)
    if birth_year:
        person.born = _event_with_year(model.Birth, birth_year)
    if death_year:
        person.died = _event_with_year(model.Death, death_year)
    return person


def _agent_people(value):
    return [
        person
        for part in _split_agent_values(value)
        if (person := _person_from_agent_value(part)) is not None
    ]


def _creator_name(value):
    person = _person_from_agent_value(value)
    return person._label if person is not None else None
...
class NhaMapper(Mapper):
    source_name = "nha"
    owner_label = NHA_LABEL
    collection_label = NHA_LABEL
    ...
    def transform(self, record, rectype=None, reference=False):
        ...
        for person_value in _values(metadata.get("persoon_op_afbeelding")):
            for person in _agent_people(person_value):
                top.about = person
        ...
        for creator_value in _values(metadata.get("vervaardiger")):
            for creator in _agent_people(creator_value):
                production.carried_out_by = creator
                has_production = True
