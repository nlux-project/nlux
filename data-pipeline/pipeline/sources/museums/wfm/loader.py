...
class WfmLoader(Loader):
    """Load Westfries Museum records from harvested Memorix JSON files."""

    def __init__(self, config):
        Loader.__init__(self, config)
        cfgs = config["all_configs"]
        self.input_dir = os.path.join(cfgs.dumps_dir, "wfm")
    ...
    def load(self):
        ...
        if not os.path.isdir(self.input_dir):
            raise FileNotFoundError(
                ...
                "Run ./harvest-wfm.sh first."
            )

        files = sorted(fn for fn in os.listdir(self.input_dir) if fn.endswith(".json"))
        total = len(files)
        print(f"WFM: loading {total} records from {self.input_dir}")
        ...
        print(f"WFM: loaded {loaded} records in {time.time() - start:.1f}s")
