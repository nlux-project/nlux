...
vi.mock('../../lib/util/fetchItems', () => ({
  ...
  // used by getAncestors to walk member_of records; a minimal non-archive
  // Set entity (no member_of of its own) stops the ancestor walk at the
  // current entity
  fetchItem: vi.fn((uri: string) =>
    Promise.resolve({ id: uri, type: 'Set', _label: 'Parent Collection' }),
  ),
}))
...
describe('Set page', () => {
  ...
  describe('About', () => {
    ...
    it('renders the about', async () => {
      ...
      const event = screen.getByTestId('about-subject-headings-row')
      expect(event).toBeInTheDocument()
    })
