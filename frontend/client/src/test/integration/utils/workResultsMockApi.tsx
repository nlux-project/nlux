...
export default function workResultsMockApi(): void {
  ...
  nock(apiUrl)
    .get(
      '/api/search/work?q=%7B%22AND%22%3A%5B%7B%22text%22%3A%22andy%22%2C%22_lang%22%3A%22en%22%7D%2C%7B%22text%22%3A%22warhol%22%2C%22_lang%22%3A%22en%22%7D%5D%7D&page=1',
    )
  ...
  // mock the api call for the second creator
  nock(apiUrl)
    .get('/data/group/created-by-carried-out-by-2?profile=name')
    .reply(200, JSON.stringify(reusableMinimalEntity('Mock Person 2')), {
      'Access-Control-Allow-Origin': '*',
      'Content-type': 'application/json',
    })
