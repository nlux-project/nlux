...
export default function setsMockApi(): void {
  ...
  // mock the api call for the archive classification
  nock(apiUrl)
    .get('/data/concept/archive?profile=name')
  ...
  // name profile for the same record
  nock(apiUrl)
    .get('/data/set/set-member-of-1?profile=name')
    .reply(200, JSON.stringify(mockArchive), {
      'Access-Control-Allow-Origin': '*',
      'Content-type': 'application/json',
    })
  ...
  // name profile for the same record
  nock(apiUrl)
    .get('/data/group/carried-out-by?profile=name')
    .reply(200, JSON.stringify(reusableMinimalEntity('Mock Group')), {
      'Access-Control-Allow-Origin': '*',
      'Content-type': 'application/json',
    })
  ...
  // name profile for the same record
  nock(apiUrl)
    .get('/data/concept/classified-as?profile=name')
    .reply(200, JSON.stringify(reusableMinimalEntity('Mock Concept')), {
      'Access-Control-Allow-Origin': '*',
      'Content-type': 'application/json',
    })
