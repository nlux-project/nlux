...
export default function sharedMock(): void {
  ...
  // name profile for the same record
  nock(apiUrl)
  ...
  // name profile for the same record
  nock(apiUrl)
    .get('/data/concept/primary-name?profile=name')
    .reply(
      200,
      JSON.stringify(reusableMinimalEntity('Primary Name', primaryNameId)),
      {
  ...
  // name profile for the same record
  nock(apiUrl)
    .get('/data/concept/display-name?profile=name')
    .reply(
      200,
      JSON.stringify(reusableMinimalEntity('Display Name', displayNameId)),
      {
  ...
  // name profile for the same record
  nock(apiUrl)
    .get('/data/concept/dutch-language?profile=name')
    .reply(
      200,
      JSON.stringify(reusableMinimalEntity('Dutch', dutchLanguageId)),
      {
  ...
  // name profile for the same record
  nock(apiUrl)
    .get('/data/concept/imprint-statement?profile=name')
    .reply(
      200,
      JSON.stringify(
        reusableMinimalEntity('Imprint Statement', imprintStatementId),
  ...
  // name profile for the same record
  nock(apiUrl)
    .get('/data/concept/language-statement?profile=name')
    .reply(
      200,
      JSON.stringify(
        reusableMinimalEntity('Language Statement', languageStatementId),
  ...
  // name profile for the same record
  nock(apiUrl)
    .get('/data/concept/dimension-statement?profile=name')
    .reply(
      200,
      JSON.stringify(
        reusableMinimalEntity('Dimension Statement', dimensionStatementId),
  ...
  // name profile for the same record
  nock(apiUrl)
    .get('/data/concept/call-number?profile=name')
    .reply(
      200,
      JSON.stringify(reusableMinimalEntity('Call Number', callNumberId)),
      {
  ...
  // name profile for the same record
  nock(apiUrl)
    .get('/data/concept/access-statement?profile=name')
    .reply(
      200,
      JSON.stringify(
        reusableMinimalEntity('Access Statement', accessStatementId),
  ...
  // name profile for the same record
  nock(apiUrl)
    .get('/data/concept/nationality?profile=name')
    .reply(
      200,
      JSON.stringify(reusableMinimalEntity('American', nationalityId)),
      {
  ...
  // Mock the works-about-concept searches requested by the about lists.
  // The criteria rewrite the concept id to the lux host (see
  // lib/parse/data/helper.ts getSubjectHeadingObject).
  for (const about of ['about-1', 'about-2']) {
    nock(apiUrl)
      .get(
        `/api/search/work?q=%7B%22AND%22%3A%5B%7B%22aboutConcept%22%3A%7B%22id%22%3A%22https%3A%2F%2Flux.collections.yale.edu%2Fdata%2Fconcept%2F${about}%22%7D%7D%5D%7D`,
      )
      .reply(
        200,
        JSON.stringify({
          id: `${apiUrl}api/search/work`,
