...
export default function linguisticObjectsMockApi(): void {
  ...
  nock(apiUrl)
    .get(
      '/api/search/item?q=%7B%22AND%22%3A%5B%7B%22carried%22%3A%7B%22id%22%3A%22https%3A%2F%2Fendpoint.yale.edu%2Fdata%2Ftext%2Fmock-linguistic-object%22%7D%7D%5D%7D',
    )
  ...
  // name profile for the same record
  nock(apiUrl)
    .get('/data/place/mock-place-entity?profile=name')
    .reply(200, JSON.stringify(reusableMinimalEntity('Place')), {
      'Access-Control-Allow-Origin': '*',
      'Content-type': 'application/json',
    })
  ...
  // name profile for the same record
  nock(apiUrl)
    .get('/data/concept/created-by-classified-as-2?profile=name')
    .reply(200, JSON.stringify(reusableMinimalEntity('Classified As')), {
      'Access-Control-Allow-Origin': '*',
      'Content-type': 'application/json',
    })
  ...
  // name profile for the same record
  nock(apiUrl)
    .get('/data/concept/created-by-classified-as-1?profile=name')
    .reply(200, JSON.stringify(reusableMinimalEntity('Classified As')), {
      'Access-Control-Allow-Origin': '*',
      'Content-type': 'application/json',
    })
